  ...
  return (
        <div>
              <div className="card" style={{ marginBottom: "2rem", background: "linear-gradient(135deg, var(--primary), #60a5fa)", color: "white" }}>
                      <h1 style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>반갑습니다, {userData?.name}님!</h1>h1>
                      <p>오늘의 좌석 신청 현황을 확인하거나 새로운 예약을 진행하세요.</p>p>
                {userData?.role === "student" && (
                    <div style={{ marginTop: "0.75rem", background: "rgba(255,255,255,0.2)", borderRadius: "8px", padding: "0.5rem 1rem", display: "inline-block" }}>
                                💰 내 포인트: <strong>{(userData?.points ?? 0).toLocaleString()}P</strong>strong>
                    </div>div>
                      )}
              </div>div>
              ...
              <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.5rem" }}>
                      <div className="card" style={{ borderTop: "4px solid var(--primary)" }}>
                                <h3>현재 진행 중인 세션</h3>h3>
                                <p style={{ color: "var(--secondary)", fontSize: "0.875rem", marginTop: "0.5rem" }}>지금 참여하여 좌석을 신청할 수 있는 세션들입니다.</p>p>
                      
                                <div style={{ marginTop: "1.5rem" }}>
                                  {loading ? (
                        <p style={{ textAlign: "center", color: "var(--secondary)" }}>로딩 중...</p>p>
                      ) : openSessions.length === 0 ? (
                        <div style={{ textAlign: "center", color: "var(--secondary)", padding: "1rem" }}>현재 진행 중인 좌석 신청 세션이 없습니다.</div>div>
                      ) : (
                          ...
                          {openSessions.map(session => {
                                            ...
                                            return (
                                                                  <button
                                                                                        ...
                                                                                        <div>
                                                                                                                <span style={{ fontWeight: "600", color: statusInfo.locked ? "var(--secondary)" : statusInfo.bg }}>{session.title}</span>span>
                ...
                {userData?.role === "teacher" && (
                    <div className="card" style={{ borderTop: "4px solid var(--accent)" }}>
                                ...
                                <ul style={{ listStyle: "none", marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                              <li><button onClick={() => router.push("/admin/students")} style={{ width: "100%", textAlign: "left", padding: "0.5rem", borderRadius: "4px", border: "1px solid var(--border)", background: "none", cursor: "pointer" }}>📂 학생 계정(DB) 관리</button>button></li>li>
                                              <li><button onClick={() => router.push("/admin/classes")} style={{ width: "100%", textAlign: "left", padding: "0.5rem", borderRadius: "4px", border: "1px solid var(--border)", background: "none", cursor: "pointer" }}>🏫 수업(학급) 설정 및 학생 배정</button>button></li>li>
                                              <li><button onClick={() => router.push("/admin/layout-editor")} style={{ width: "100%", textAlign: "left", padding: "0.5rem", borderRadius: "4px", border: "1px solid var(--border)", background: "none", cursor: "pointer" }}>📐 좌석 배치도 설정</button>button></li>li>
                                              <li><button onClick={() => router.push("/admin/sessions")} style={{ width: "100%", textAlign: "left", padding: "0.5rem", borderRadius: "4px", border: "1px solid var(--border)", background: "none", cursor: "pointer" }}>⏰ 예약 세션 관리</button>button></li>li>
                                              <li><button onClick={() => router.push("/admin/users")} style={{ width: "100%", textAlign: "left", padding: "0.5rem", borderRadius: "4px", border: "1px solid var(--border)", background: "none", cursor: "pointer", color: "var(--primary)", fontWeight: "bold" }}>💰 학생 실시간 포인트 현황/부여</button>button></li>li>
                                </ul>ul>
                    </div>div>
                      )}
              </section>section>
