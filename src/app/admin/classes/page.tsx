...
export default function ClassManagement() {
  ...
  const saveModalAssignment = async () => {
          ...
          try {
                    await updateDoc(doc(db, "classes", selectedClass.id), { studentIds: tempStudentIds });
                    setModalOpen(false);
  ...
  if (loading) return <div className="card">로딩 중...</div>div>;
                          ...
                          <div
                                    ...
                                    <div
                                                ...
                                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                                                              <h3 style={{ margin: 0 }}>개별 배정 관리 – {selectedClass.name}</h3>h3>
                                                              <button
                                                ...
                                                <div style={{ overflowY: "auto", flex: 1, border: "1px solid #e2e8f0", borderRadius: "6px" }}>
                                                    {filteredStudents.length === 0 ? (
                                                                      <div style={{ padding: "1rem", color: "#94a3b8", textAlign: "center" }}>검색 결과가 없습니다.</div>div>
                                                                    ) : (
                                                                                            ...
                                                                                            return (
                                                                                                                    <label
                                                                                                                                          ...
                                                                                                                                          <input
                                                                                                                                                                    ...
                                                                                                                                                                    />
                                                                                                                                          <span style={{ fontWeight: checked ? "bold" : "normal" }}>{student.name}</span>span>
                                                                                                                                          <span style={{ color: "#64748b", fontSize: "0.8rem" }}>({student.id})</span>span>
                                                                                                                        </label>label>
                                                                                                                  );
                                                                      })
                                                                    )}
                                                ...
                                                <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem", justifyContent: "flex-end" }}>
                                                              <button
                                                                                  onClick={() => setModalOpen(false)}
                                                                                  style={{ padding: "0.5rem 1.25rem", borderRadius: "4px", border: "1px solid #e2e8f0", background: "white", cursor: "pointer" }}
                                                                                >
                                                                              취소
                                                              </button>button>
                                                              <button
                                                                                  onClick={saveModalAssignment}
                                                                                  style={{ padding: "0.5rem 1.25rem", borderRadius: "4px", border: "none", background: "var(--primary)", color: "white", fontWeight: "bold", cursor: "pointer" }}
                                                                                >
                                                                              저장
                                                              </button>button>
                                                </div>div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                            <div className="card">
                                ...
                                <div key={cls.id} className="card" style={{ border: "1px solid var(--border)", padding: "1.25rem" }}>
                                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                                                                  <h3 style={{ margin: 0 }}>{cls.name}</h3>h3>
                                                                  <button
                                                ...
                                                <div style={{ marginBottom: "1rem" }}>
                                                                  ...
                                                                  <div style={{
                                                        maxHeight: "150px", overflowY: "auto", fontSize: "0.875rem",
                                                        background: "#f8fafc", padding: "0.5rem", borderRadius: "4px",
                                                        display: "flex", flexWrap: "wrap", gap: "0.4rem",
                                }}>
                                                                      {cls.studentIds.length === 0 ? (
                                                          <span style={{ color: "#94a3b8" }}>배정된 학생이 없습니다.</span>span>
                                                        ) : (
                                                ...
                                                <div style={{ display: "flex", gap: "0.5rem" }}>
                                                                  <div style={{ flex: 1, position: "relative" }}>
                                                                                      <button
                                                                                                                style={{ width: "100%", padding: "0.5rem", fontSize: "0.8125rem", background: "white", border: "1px solid var(--border)", borderRadius: "4px", cursor: "pointer" }}
                                                                                                                onClick={() => { const el = document.getElementById(`file-${cls.id}`); el?.click(); }}
                                                                                                              >
                                                                                                            📥 엑셀로 일괄 배정
                                                                                          </button>button>
                                                                                      <input
                                                                                                                ...
                                                                                                                accept=".xlsx, .xls"
                                                                                                                style={{ display: "none" }}
                                                                                                                onChange={(e) => { const file = e.target.files?.[0]; if (file) handleBulkAssign(cls.id, file); }}
                                                                                                              />
                                                                  </div>div>
                                                                  <button
                                                                                          style={{ flex: 1, padding: "0.5rem", fontSize: "0.8125rem", background: "white", border: "1px solid var(--border)", borderRadius: "4px", cursor: "pointer" }}
                            ...
                            <div className="card">
                                                  ...
                                                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
                                                                <thead>
                                                                                <tr style={{ background: "#f8fafc", borderBottom: "2px solid var(--border)" }}>
                                                                                                  <th style={{ padding: "0.75rem", textAlign: "left" }}>학번</th>th>
                                                                                                  <th style={{ padding: "0.75rem", textAlign: "left" }}>이름</th>th>
                                                                                    {classes.map((cls) => (
                                      <th key={cls.id} style={{ padding: "0.75rem", textAlign: "center" }}>{cls.name}</th>th>
                                    ))}
                                                                                </tr>tr>
                                                                </thead>thead>
                                                                <tbody>
                                    ...
                                    <tr key={student.id} style={{ borderBottom: "1px solid var(--border)" }}>
                                                        <td style={{ padding: "0.75rem" }}>{student.id}</td>td>
                                                        <td style={{ padding: "0.75rem", fontWeight: "bold" }}>{student.name}</td>td>
                                        {classes.map((cls) => {
